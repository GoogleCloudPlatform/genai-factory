...
def main():
  parser = argparse.ArgumentParser(
      description=
      "Clones a Git repository to find and display the most recent version tag.",
  ...
  parser.add_argument('repository_url', metavar='REPO_URL', type=str,
  ...
  try:
    ...
    subprocess.run([
        'git', 'clone', '--quiet', '--no-checkout', '--depth', '1', repo_url,
        temp_dir
    ], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

    # Fetch all tags from the remote repository
    subprocess.run(['git', 'fetch', '--quiet', '--tags'], cwd=temp_dir,
                   check=True, stdout=subprocess.DEVNULL,
                   stderr=subprocess.DEVNULL)

    # Get all tags starting with 'v', sorted by version number (for display)
    git_list_proc = subprocess.Popen(['git', 'tag', '--list', 'v*'],
                                     cwd=temp_dir, stdout=subprocess.PIPE)
    sort_proc = subprocess.run(['sort', '-V'], stdin=git_list_proc.stdout,
    ...
    if args.sort_by_date:
      ...
      latest_tag_proc = subprocess.run(
          ['git', 'tag', '--list', 'v*', '--sort=-committerdate'], cwd=temp_dir,
          capture_output=True, text=True, check=True)
      ...
      latest_tag_proc = subprocess.run(
          ['git', 'tag', '--list', '--sort=-v:refname'], cwd=temp_dir,
          capture_output=True, text=True, check=True)
