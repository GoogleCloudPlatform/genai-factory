...
async def sanitize_request(callback_context: CallbackContext,
    ...
    if not config.MODEL_ARMOR_TEMPLATE:
        logger.warning("MODEL_ARMOR_TEMPLATE environment variable not set. Skipping prompt inspection.")
        return None
    ...
    try:
        ...
        if matched_filters:
            ...
            return LlmResponse(content={"parts": [{"text": "{\"capital\": \"Blocked by Model Armor\", \"population_estimate\": \"N/A\"}"}], "role": "assistant"})
        else:
