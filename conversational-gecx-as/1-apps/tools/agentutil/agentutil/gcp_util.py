...
def get_current_user_email(credentials) -> str:
    ...
    try:
            ...
            r = requests.get("https://www.googleapis.com/oauth2/v3/userinfo", headers={"Authorization": f"Bearer {credentials.token}"})
    ...
    try:
        email = subprocess.check_output(["gcloud", "config", "get-value", "account"], text=True).strip()
        if email: return email
    except:
        pass
        
    logger.warning("Could not determine current user email. Permission check might differ from script.")
    return "unknown"

def has_sa_user_role(sa_email: str, user_email: str, project_id: str | None, credentials) -> bool:
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            logger.warning(f"Failed to test permissions for {sa_email}: {r.status_code} {r.text}")
