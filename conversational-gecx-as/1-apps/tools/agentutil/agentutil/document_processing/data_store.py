...
def process_data_store_documents(
        ...
        logger.info(f"Parsed GCS path: Bucket='{gcs_bucket_name}', Blob Prefix='{gcs_blob_prefix}'")
        ...
        for filename in track(os.listdir(source_dir)):
            ...
            try:
                ...
                logger.info(f"    - Prepared JSONL entry for ID: '{base_name}' (Title: '{title}')")
...
def import_documents(full_data_store_name: str, gcs_uri: str) -> None:
    ...
    if not match:
        raise ValueError(f"Could not extract location from data store name: '{full_data_store_name}'")
    ...
    body = {
        "gcsSource": {
            "inputUris": [gcs_uri],
            "dataSchema": "document" # OR "custom" depending on setup, but typically "document" for Unstructured/structured
        },
    ...
    if response.status_code != 200:
        logger.error(f"Import failed with status code {response.status_code}: {response.text}")
        response.raise_for_status()
    
    logger.info(f"Import started. Response: {json.dumps(response.json(), indent=2)}")
