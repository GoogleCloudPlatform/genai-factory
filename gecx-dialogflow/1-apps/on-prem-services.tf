...
module "service_directory" {
  ...
  source     = "github.com/GoogleCloudPlatform/cloud-foundation-fabric//modules/service-directory?ref=v58.0.0"
  ...
  endpoint_config = {
    ...
    : k => {
      address = (
        v.create_lb
        ? module.lbs-int-proxy["${each.key}-${k}"].address["${each.key}-${k}"]
        : v.address
      )
      port     = v.port
      metadata = v.metadata
...
module "lbs-int-proxy" {
  ...
  source     = "github.com/GoogleCloudPlatform/cloud-foundation-fabric//modules/net-lb-proxy-int?ref=v58.0.0"
  name       = each.key
  project_id = var.project_id
  region     = var.region
