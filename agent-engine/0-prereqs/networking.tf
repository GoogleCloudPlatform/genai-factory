...
module "vpc" {
  ...
  source     = "github.com/GoogleCloudPlatform/cloud-foundation-fabric//modules/net-vpc?ref=v56.3.0"
...
module "dns_policy_googleapis" {
  count      = var.networking_config.create ? 1 : 0
  source     = "github.com/GoogleCloudPlatform/cloud-foundation-fabric//modules/dns-response-policy?ref=v56.3.0"
  project_id = module.projects.projects["host"].project_id
  name       = "googleapis"
  factories_config = {
...
module "secure-web-proxy" {
  count        = var.networking_config.create ? 1 : 0
  source       = "github.com/GoogleCloudPlatform/cloud-foundation-fabric//modules/net-swp?ref=v56.3.0"
  project_id   = module.projects.projects["host"].project_id
  region       = var.region
  name         = "proxy-0"
