...
locals {
  ...
  network_attachment_id = try(
    google_compute_network_attachment.network_attachment[0].id,
    "projects/${var.networking_config.host_project_id}/regions/${var.region}/networkAttachments/${var.networking_config.network_attachment_name}",
    ""
  )
  networking_config = {
    subnet = coalesce(
  ...
  tfvars = {
    ...
    project_id            = local.projects["service-01"].id
    project_number        = local.projects["service-01"].number
    proxy_ip              = var.networking_config.proxy_config.ip_address
    proxy_port            = var.networking_config.proxy_config.port
    region                = var.region
