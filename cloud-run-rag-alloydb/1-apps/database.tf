...
module "bigquery-dataset" {
  source     = "github.com/GoogleCloudPlatform/cloud-foundation-fabric//modules/bigquery-dataset?ref=v56.0.0"
...
module "alloydb" {
  source              = "github.com/GoogleCloudPlatform/cloud-foundation-fabric//modules/alloydb?ref=v56.0.0"
  project_id          = var.project_id
  ...
  deletion_protection = var.enable_deletion_protection
  network_config = {
    psc_config = { allowed_consumer_projects = [var.number] }
  }
  users = {
    ...
    (trimsuffix(var.service_account_emails["service-01/gf-rrag-fe-0"], ".gserviceaccount.com")) = {
      type     = "ALLOYDB_IAM_USER"
      roles    = ["alloydbiamuser"]
      password = null # password will be generated
    }
    (trimsuffix(var.service_account_emails["service-01/gf-rrag-ing-0"], ".gserviceaccount.com")) = {
      type     = "ALLOYDB_IAM_USER"
...
module "psc_consumer_address" {
  source     = "github.com/GoogleCloudPlatform/cloud-foundation-fabric//modules/net-address?ref=v56.0.0"
  project_id = var.project_id
  internal_addresses = {
    (var.name) = {
      region     = var.region
...
resource "google_compute_forwarding_rule" "psc_consumer_fwd_rule" {
  ...
  ip_address              = module.psc_consumer_address.internal_addresses[var.name].id
...
resource "google_dns_record_set" "psc_consumer_dns_record" {
  ...
  rrdatas      = [module.psc_consumer_address.internal_addresses[var.name].address]
}
