...
locals {
  address_ilb = try(coalesce(
    ...
    module.address-ilb[0].internal_addresses["ilb-01"].address
  ), null)
}
...
module "lb_internal_redirect" {
  count                = var.lbs_configs.internal.enable ? 1 : 0
  source               = "github.com/GoogleCloudPlatform/cloud-foundation-fabric//modules/net-lb-app-int?ref=v58.0.0"
...
module "certificate_manager" {
  count      = var.lbs_configs.internal.enable ? 1 : 0
  source     = "github.com/GoogleCloudPlatform/cloud-foundation-fabric//modules/certificate-manager?ref=v58.0.0"
  project_id = var.project_id
  certificates = {
    (var.name) = {
      location = var.region
