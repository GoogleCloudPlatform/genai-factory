...
resource "google_compute_network_attachment" "agent_gateway" {
  ...
  # The producer acceptance list is automatically populated by the service agent
  # that's why we granted network admin to it in the service project
  # Reverting the list would remove Google's tenant project,
  # severing the Agent Gateway's connection to your subnet.
  lifecycle {
