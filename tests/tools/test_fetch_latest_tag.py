...
def mock_sys_argv():
  with patch.object(sys, 'argv',
                    ['fetch_latest_tag.py', 'http://example.com/repo.git']):
    yield
...
def test_main_sort_by_version(mock_popen, mock_run, mock_rmtree, mock_mkdtemp,
                              capsys, mock_sys_argv):
  # Mock temp directory
  mock_mkdtemp.return_value = '/tmp/testdir'

  # Mock git tag list process
  mock_git_list_proc = MagicMock()
  mock_git_list_proc.stdout.strip.return_value.splitlines.return_value = [
      'v1.0.0', 'v1.1.0', 'v2.0.0'
  ]

  # Mock sort process
  mock_sort_proc = MagicMock()
  mock_sort_proc.stdout.strip.return_value.splitlines.return_value = [
      'v1.0.0', 'v1.1.0', 'v2.0.0'
  ]

  # Mock latest tag process
  mock_latest_tag_proc = MagicMock()
  mock_latest_tag_proc.stdout.strip.return_value.splitlines.return_value = [
...
@patch('tools.fetch_latest_tag.subprocess.run')
@patch('tools.fetch_latest_tag.subprocess.Popen')
def test_main_sort_by_date(mock_popen, mock_run, mock_rmtree, mock_mkdtemp,
                           capsys):
  # Mock command line arguments
  with patch.object(
      sys, 'argv',
      ['fetch_latest_tag.py', '--sort_by_date', 'http://example.com/repo.git']):
    # Mock temp directory
    mock_mkdtemp.return_value = '/tmp/testdir'
    ...
    mock_latest_tag_proc_date.stdout.strip.return_value.splitlines.return_value = [
        'v1.1.0', 'v2.0.0', 'v1.0.0'
    ]
...
def test_main_git_command_fails(mock_rmtree, mock_mkdtemp, capsys,
                                mock_sys_argv):
  mock_mkdtemp.return_value = '/tmp/testdir'
